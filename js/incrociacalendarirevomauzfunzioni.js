...
//prima intestazione
document.write("<table align=center width=848px' class='ClassTabellaincroci' cellpadding=0 cellspacing=0>")
//prima intestazione
//secondaintestazione
document.write("<tr class='rigasquadre'><td class='t-xxsNero' rowspan='3'>Nome</td>")
for (i=1;i<tableIncrociTeams.length;i++){
	document.write("<td class='t-xxsNero'>"+tableIncrociTeams[i].substring(0,11)+"</td>")
}
document.write("</tr><tr class='rigasquadre'></tr><tr class='rigasquadre'></tr>")
for (i=1;i<tableIncrociTeams.length;i++){	
	document.write("<tr class='rigaincrocipari'>")
	
	document.write("<td class='cellasquadra'>"+tableIncrociTeams[i].substring(0,11)+"</td>")
	for (j=1;j<tableIncrociTeams.length;j++){
	...
	document.write("<tr><td colspan='6'>&nbsp</td><td colspan='"+(tableIncrociTeams.length/2+1)+"'></td></tr>")		
	document.write("<tr><td colspan='6'><img src='img/invert_stat_txt.png'></td><td colspan='"+(tableIncrociTeams.length/2+1)+"'></td></tr>")		
	document.write("<tr class='rigaintestazione'><td colspan='"+(tableIncrociTeams.length)+"'></td></tr>")	
	j=1
	buffer1=media[1]
	buffer2="Squadra piu' fortunata nell'assegnazione calendario:"
	while (media[j]==buffer1){
	if(j>1)	{
	buffer2="" 	
	}	
	document.write("<tr class='rigaincrociopari'><td class='cellarec' colspan='6'>"+buffer2+"</td>")		
	document.write("<td class='t-xxsNerox' colspan='"+(tableIncrociTeams.length/2+1)+"'>"+nomitassodisfiga[j]+"</td></tr>")
	j++
	}
	j=1
	buffer1=media[tableIncrociTeams.length-j]
	buffer2="Squadra piu' sfortunata nell'assegnazione calendario:"
	while (media[tableIncrociTeams.length-j]==buffer1){
	if(j>1)	{
	buffer2="" 	
	}	
	document.write("<tr class='rigaincrociopari'><td class='cellarec' colspan='6'>"+buffer2+"</td>")	
	document.write("<td class='t-xxsNerox' colspan='"+(tableIncrociTeams.length/2+1)+"'>"+nomitassodisfiga[tableIncrociTeams.length-j]+"</td></tr>")
	j++
	}
	j=1
	buffer1=calendariokiller[1]
	buffer2="Calendario piu' facile in assoluto:"
	while (calendariokiller[j]==buffer1){
	if(j>1)	{
	buffer2="" 	
	}	
	document.write("<tr class='rigaincrociopari'><td class='cellarec' colspan='6'>"+buffer2+"</td>")		
	document.write("<td class='t-xxsNerox' colspan='"+(tableIncrociTeams.length/2+1)+"'>"+nomicalendario[j]+"</td></tr>")
	j++
	}
	j=1
	buffer1=calendariokiller[tableIncrociTeams.length-j]
	buffer2="Calendario piu' difficile in assoluto:"
	while (calendariokiller[tableIncrociTeams.length-j]==buffer1){
	if(j>1)	{
	buffer2="" 	
	}	
	document.write("<tr class='rigaincrociopari'><td class='cellarec' colspan='6'>"+buffer2+"</td>")	
	document.write("<td class='t-xxsNerox' colspan='"+(tableIncrociTeams.length/2+1)+"'>"+nomicalendario[tableIncrociTeams.length-j]+"</td></tr>")
	j++
	}
