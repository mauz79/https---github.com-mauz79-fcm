...
var i,iD,isDivisione=0
	...
	tabS.SetStileRiga(isDivisione+3,"t-xxs")
	tabS.SetStile(isDivisione+3,1,"IntRossoBluDx")
	tabS.SetValore(isDivisione+3,1,"Crediti Residui:&nbsp;")
	tabS.SetStile(isDivisione+3,2,"Cella")
	tabS.SetValore(isDivisione+3,2,"&nbsp;" + arrFantasquadre[i].CreditiResidui)
	
		tabS.SetStileRiga(isDivisione+4,"t-xxs")
	tabS.SetStile(isDivisione+4,1,"IntRossoBluDx")
	tabS.SetValore(isDivisione+4,1,"Logo Squadra:&nbsp;")
	tabS.SetStile(isDivisione+4,2,"Cella")
	tabS.SetValore(isDivisione+4,2,'&nbsp; <img src="./img/loghi/'+arrFantasquadre[i].Nome+'.jpg" alt="Logo '+arrFantasquadre[i].Nome+'"></img>')
	tabS.SetStileRiga(isDivisione+5,"t-xxs")
	tabS.SetStile(isDivisione+5,1,"IntRossoBluDx")
	tabS.SetValore(isDivisione+5,1,"Maglia :&nbsp;")
	tabS.SetStile(isDivisione+5,2,"Cella")
	tabS.SetValore(isDivisione+5,2,'&nbsp;<img src="./img/sq/lega/'+arrFantasquadre[i].Nome+'.gif" alt="Maglia '+arrFantasquadre[i].Nome+'"></img>')
	tabS.SetStileRiga(isDivisione+6,"t-xxs")
	tabS.SetStile(isDivisione+6,1,"IntRossoBluDx")
	tabS.SetValore(isDivisione+6,1,"Sponsor :&nbsp;")
	tabS.SetStile(isDivisione+6,2,"Cella")
	tabS.SetValore(isDivisione+6,2,'&nbsp;<img src="./img/sponsor/'+arrFantasquadre[i].Nome+'.jpg" alt="Sponsor '+arrFantasquadre[i].Nome+'"></img>')
	tabS.SetStileRiga(isDivisione+7,"t-xxs")
	tabS.SetStile(isDivisione+7,1,"IntRossoBluDx")
	tabS.SetValore(isDivisione+7,1,"eMail:&nbsp;")
	tabS.SetStile(isDivisione+7,2,"Cella")
	tabS.SetValore(isDivisione+7,2,"&nbsp;<a href='mailto:" + arrFantasquadre[i].Email+"'>"+arrFantasquadre[i].Email+"</a>")
	tabS.SetStileRiga(isDivisione+8,"t-xxs")
	tabS.SetStile(isDivisione+8,1,"IntRossoBluDx")
	tabS.SetValore(isDivisione+8,1,"Stadio :&nbsp;")
	tabS.SetStile(isDivisione+8,2,"Cella")
	tabS.SetValore(isDivisione+8,2,'&nbsp;<img src="./img/stadio/'+arrFantasquadre[i].Nome+'.jpg" alt="Stadio '+arrFantasquadre[i].Nome+'"></img>')
	...
	tabS.cellspacing=0
	tabS.stile="ClassEl"
	tabS.SetLarghezzaColonna(1,47)
	tabS.SetLarghezzaColonna(2,48)
	tabS.Stampa()
}
...
var i,cnt,sT,sR,fatto,p,photo
	...
	tabR.SetLarghezzaColonna(2,10)
	tabR.SetLarghezzaColonna(3,60)
	tabR.SetLarghezzaColonna(4,20)
	tabR.SetLarghezzaColonna(5,4)
	tabR.SetLarghezzaColonna(6,6)
	...
	tabR.SetStile(1,6,"Cella")
	tabR.SetValore(1,1,"Foto")
	tabR.SetValore(1,2,"Ruolo")
	tabR.SetValore(1,3,"Nome")
	tabR.SetValore(1,4,"Squadra")
	tabR.SetValore(1,5,"Con")
	tabR.SetValore(1,6,"$Acq")
	cnt=2
	for(i=1;i<arrRose.length;i++) {
		if ((arrRose[i].IDSquadra==IDSquadra)&&(arrRose[i].Stato==0)) {
			...
			tabR.SetStile(cnt,3,"CellaCentro")
			tabR.SetStile(cnt,4,"CellaCentro")
			tabR.SetStile(cnt,5,"CellaCentro")
			tabR.SetStile(cnt,6,"CellaCentro")
			sT="<span class='t-xxs"
			...
			// INIZIO MODIFICA PUFFIN PHOTOPATCH 2008
			// Questo è il core della modifica, ovviamente
			// dovete aggiungere una colonna alla tabella
			// in cui volete visualizzare le foto.
			for(p=1;p<arrGiocatoriA.length;p++) {
				 check = ("xg"+arrGiocatoriA[p].ID)
			   if (eval(check) == arrRose[i].Nome) {
				 photo=arrGiocatoriA[p].Codice
				 }
			}

			tabR.SetValore(cnt,1,'<img src="img/foto/'+photo+'.jpg" onError=this.src="img/foto/misterx.gif"></img>')
			// FINE MODIFICA PUFFIN PHOTOPATCH 2008
			tabR.SetValore(cnt,2,sT)
			tabR.SetValore(cnt,3,"<span class='t-xxs"+sR+"B'>&nbsp;"+arrRose[i].Nome+"</span>")
			tabR.SetValore(cnt,4,"<span class='t-xxs"+sR+"'>&nbsp;"+arrRose[i].Squadra+"</span>")
			tabR.SetValore(cnt,5,"<span class='t-xxs"+sR+"'>"+(arrRose[i].Contratto==-2?"Pres":arrRose[i].Contratto)+"</span>")
			tabR.SetValore(cnt,6,"<span class='t-xxs"+sR+"B'>"+arrRose[i].Acq+"</span>")
			cnt++
		} else {
...
var i,cnt,sT,sR,fatto
	...
	for(i=1;i<arrBilanci.length;i++) {
		if (arrBilanci[i].IDSquadra==IDSquadra) {
			...
			tabB.SetStile(cnt,4,"CellaCentro")
	...
	tabB.SetStileRiga(cnt,"IntRossoBlu")
	tabB.SetStile(cnt,1,"Cella")
