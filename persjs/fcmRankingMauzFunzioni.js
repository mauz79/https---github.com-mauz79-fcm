...
function Rank(IdAlbo,IdAttuale,Squadra,Punti,Stagioni,PuntiPrecedenti) {
        this.IdAlbo = IdAlbo
...
document.write("<form name='frmFsq' id='frmFsq' action='ranking.php' method='get'>")
...
var rankingPrec = new Array()
ranking = arrRanking
rankingPrec = arrRanking.slice(0)
        
        ranking.sort(ordinaRanking)
        rankingPrec.sort(ordinaRankingPrec)
	fatto=false
	...
	for(i=0;i<ranking.length-1;i++) {
		...
		if ((cnt % 2) != 0) {
			...
			tabR.SetStileRiga(cnt,"riga-dispari")
                ...
                sR="LFB"
                tabR.SetValore(cnt,1,"<nobr><center>"+c+"</center></nobr>")
